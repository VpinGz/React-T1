...
const CustomerForm = () => {
  ...
  useEffect(() => {
    ...
    dispatch(addCustomerById(customer_form));
    
  }, [customer_form]);

  const [customer_form, setCustomerSignUp] = useState(
    { email: '', password: '', firstName: '', lastName: ''}
  );

  const handleChange = (event) => {
    setCustomerSignUp({...customer_form, [event.target.name]: event.target.value})
  }

  const handleSubmit = (e) => {
    e.preventDefault()
    axios.post(`${baseApiUrl}/customer`, customer_form)
